...
void calculateMatricesMultiplicationSequentially(Matrix *matrix1, Matrix *matrix2) {
  cout << "Init calculateMatricesMultiplicationSequentially \n";
  ...
  cout << "End calculateMatricesMultiplicationSequentially \n";
}

void handleMatricesSequentially() {
...
void calculateMatricesMultiplicationParallelThread(Matrix *matrix1, Matrix *matrix2) {
  int r1 = matrix1->getRow(), c1 = matrix1->getCol(), r2 = matrix2->getRow(), c2 = matrix2->getCol(), i, j, k;
  ...
  for (i = 0; i < r1; i++) {
    for (j = 0; j < c2; j++) {
      ...
      for (k = 0; k < c1; k++) {
        data[k + 1] = matrix1->getElementValue(i, k);
      }

      for (k = 0; k < r2; k++) {
        data[k + c1 + 1] = matrix2->getElementValue(k, j);
      }
...
void handleMatricesWithParallelThread() {
  Matrix *matrix1 = loadMatrix("m1.txt");
  Matrix *matrix2 = loadMatrix("m2.txt");

  calculateMatricesMultiplicationParallelThread(matrix1, matrix2);
...
void listAvailableCommands() {
  ...
  cout << "rms                                            Start reading and calculating sequentially matrices \n";
  cout << "rmpt                                           Start reading and calculating parallel thread matrices \n";
  cout << "\n";
}
  ...
  do {
    ...
    try {
      if (strcmp(arguments, "quit") == 0) {
        ...
        cout << "\n";
        writerMatrix();
      } else if (strcmp(arguments, "rms") == 0) {
        cout << "\n";
        handleMatricesSequentially();
      } else if (strcmp(arguments, "rmpt") == 0) {
        cout << "\n";
        handleMatricesWithParallelThread();
